...
export default function CalibrationFlow({ vacancies }: { vacancies: BlindVacancy[] }) {
  ...
  const submitReason = () => {
    if (submitting.current || reasonSaved) return; if (!reason) { setError("Kies eerst een reden."); return; } submitting.current = true;
  ...
  return <div className="calibration"><header className="calibration-head"><div><p className="eyebrow">Blind beoordelen</p></div><b>{index + 1} van {vacancies.length}</b></header><div className="progress" aria-label={`${index + 1} van ${vacancies.length}`}><span style={{ width: `${(index + 1) / vacancies.length * 100}%` }}/></div><VacancyReviewDetail vacancy={vacancy} review={review} concealAssessment={!reveal} assessment={reveal}/></div>;
}
