...
import { and, asc, desc, eq, ilike, inArray, isNotNull, isNull, sql, SQL } from "drizzle-orm";
...
export default async function Page({ searchParams }: { searchParams: Promise<Q> }) {
  ...
  const filters: SQL[] = [eq(vacancies.active, true)];
  if (q.city) filters.push(ilike(vacancies.location, `%${q.city}%`));
  if (q.employer) filters.push(ilike(vacancies.employer, `%${q.employer}%`));
  if (q.source) filters.push(eq(sources.slug, q.source));
  if (q.salary === "known") filters.push(isNotNull(vacancies.salaryMin));
  if (q.salary === "unknown") filters.push(isNull(vacancies.salaryMin));
  if (q.feedback === "unreviewed") filters.push(isNull(feedback.id));
  if (q.feedback && q.feedback !== "unreviewed") filters.push(eq(feedback.value, q.feedback as keyof typeof verdictLabels));
  if (q.ai === "unassessed") filters.push(isNull(aiAssessments.id));
  if (q.ai === "promising") filters.push(inArray(aiAssessments.verdict, promisingAiVerdicts));
  if (q.ai && q.ai !== "unassessed" && q.ai !== "promising") filters.push(eq(aiAssessments.verdict, q.ai as keyof typeof verdictLabels));
  const ordering = q.sort === "deadline" ? asc(vacancies.deadline) : q.sort === "ai-score" ? sql`case when ${aiAssessments.verdict} = 'interesting' then 0 when ${aiAssessments.verdict} = 'maybe' then 1 else 2 end, ${aiAssessments.score} desc nulls last, ${vacancies.firstSeenAt} desc` : desc(vacancies.firstSeenAt);
  ...
  return <><div className="page-title"><div><p className="eyebrow">Volledige actieve vacaturelijst</p><h1>Alle vacatures</h1></div><span className="muted">{rows.length} resultaten</span></div><form className="filters">
